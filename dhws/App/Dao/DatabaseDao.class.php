...
class DatabaseDao { 
	...
	function selecti($sqlString,$resultType){
	 
		$mysqliObj = new mysqli('localhost','root','123456','hzremote');
		if(mysqli_connect_errno()){
			echo "连接失败".mysqli_connect_error();
			exit();
		}
		$data = mysql_fetch_object($sqlString);
		printf($data);
		/*if ($result = $mysqliObj->query($sqlString)) {
			$row = $result->fetch_row();
			printf("JSON is %s!\n", $row[0]);
		} else {
			printf("Errormessage: %s\n", $mysqliObj->error());
		}*/
		//$data = $mysqliObj->query($sqlString);
		$mysqliObj->close(); 
