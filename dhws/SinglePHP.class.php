...
class SinglePHP {
    ...
    public function run(){
		...
		if($this->c == "index" || $this->c == "Index" ||$this->c == "api" || $this->c == "Api" || $this->c == "other" || $this->c == "Other"){ 
...
class Log{
     ...
     */
	public static function write($msg, $level='DEBUG', $wf=false,$file){  
		if(function_exists('sae_debug')){ //如果是SAE，则使用sae_debug函数打日志
			...
			$logPath = C('APP_FULL_PATH').'/Log/'.date('Ymd').$file.'.log'; 
			if($wf){
				$logPath .= '.wf';
			}
